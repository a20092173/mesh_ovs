...
class WaitingPath (object):
  ...
  def notify (self, event):
    ...
    if len(self.xids) == 0:
      ...
      core.l2_multi.raiseEvent(PathInstalled(mesh_path))
    ...
    def change():
     ...
     for i in mesh_path.keys():
       ...
       if  i is not 1 and i is not len(mesh_path.keys()):  #the first is srchost the last is aimhost , other is router 
         ...
         msg.match.nw_src = EthAddr(mesh_path[1][0]) ##because we match src mac, so we dont change src mac below
         ...
         if i is mesh_len:
           msg.actions.append(of.ofp_action_dl_addr.set_dst(EthAddr(mesh_path[i+1][0])))
         else:        
